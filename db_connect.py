...
class REPORT_DB():
    ...
    def delite_prod(self, progect: str):
        ...
        result = self.cursor.execute("""DELETE FROM progect_main WHERE progect_name = ?""",
                                     (progect,)) and self.connection.commit()
        return result
    ...
    def get_info_ab_day_report(self, prog_name):
        ...
        self.cursor.execute("""SELECT * FROM report_for_progect_2 WHERE progect_name = ? ORDER BY real_date ASC""",
                            (prog_name,))
        return self.cursor.fetchall()
    ...
    def add_report_spu(self, prog_name, date, raskroy_polypr, raskroy_py, gloe_polypr,
        ...
        return self.cursor.execute("""INSERT INTO report_for_spu (progect_name, data_spu, raskroy_polypr, raskroy_py,
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""", (prog_name, date, raskroy_polypr, raskroy_py,
                                               gloe_polypr, punch_luverc, ready_prod, common_proc,
    ...
    def get_info_ab_day_report_spu(self, prog_name):
        ...
        self.cursor.execute("""SELECT * FROM report_for_spu WHERE progect_name = ? ORDER BY real_date ASC""",
                            (prog_name,))
        return self.cursor.fetchall()
    ...
    def get_tent_day_report_spu(self, prog_name, date):
        ...
        self.cursor.execute(
            """SELECT * FROM report_for_spu WHERE progect_name = ? AND data_spu = ? ORDER BY real_date ASC""",
            (prog_name, date,))
        return self.cursor.fetchall()
    ...
    def delit_report_spu_data(self, progect_name=str, data=str):
        ...
        result = self.cursor.execute("""DELETE FROM report_for_spu WHERE progect_name = ? and data_spu = ?""",
                                     (progect_name, data,)) and self.connection.commit()
        return result

    def add_report_pvh(self, prog_name=str, date='dd-mm-yyyy', raskroy_polotna=str, raskroy_pockets=str, nashelnik=str,
                       weld_pockets=str, weld_in_pockets=str, stitching=str,
    ...
    def get_info_ab_day_report_pvh(self, prog_name):
        ...
        self.cursor.execute("""SELECT * FROM report_for_pvh WHERE progect_name = ? ORDER BY real_date ASC""",
                            (prog_name,))
        return self.cursor.fetchall()
    ...
    def add_plan_for_mounth(self, date, plan_for_kmd, plan_for_tent, plan_spu, data_real):
        ...
        return self.cursor.execute("""INSERT INTO mounth_plan (mounth_year, plan_kmd, plan_tent, plan_spu, data_real)
         VALUES (?, ?, ?, ?, ?)""",
                                   (date, plan_for_kmd, plan_for_tent, plan_spu,
                                    data_real,)) and self.connection.commit()

    def delite_plan(self, date=str):
        """Удаляем информацию о плане"""
        result = self.cursor.execute("""DELETE FROM mounth_plan WHERE mounth_year = ?""",
                                     (date,)) and self.connection.commit()
        return result

    def update_plan_for_mounth(self, plan_kmd, plan_tent, plan_spu, date):
        """Обновляем Актуальность проекта ПВХ"""
        result = self.cursor.execute(
            """UPDATE mounth_plan SET plan_kmd = ? AND plan_tent = ? AND plan_spu = ?WHERE data_real = ?""",
    ...
    def add_montag_everyday_report(self, progect_name, date, org_work, instl_metal_frame, instl_fenc_constr,
                                   instl_engin_syst,
    ...
    def get_info_evr_report_montage(self, prog_name):
        """Достаем всю информацию из таблицы готовности ПВХ"""
        self.cursor.execute("""SELECT * FROM report_for_montage WHERE progect_name = ? ORDER BY real_date ASC""",
                            (prog_name,))
        return self.cursor.fetchall()
    ...
    def get_info_report_montage(self, prog_name, real_date):
        """Достаем всю информацию из таблицы готовности ПВХ"""
        self.cursor.execute(
            """SELECT * FROM report_for_montage WHERE progect_name = ? and real_date = ? ORDER BY real_date ASC""",
            (prog_name, real_date,))
        return self.cursor.fetchall()
    ...
    def get_info_about_today_montage_report(self, key, date='dd MMM yy'):
        """Достаем всю информацию из таблицы готовности КМД"""
        self.cursor.execute(
            """SELECT progect_name, common_pecr_of_work FROM report_for_montage WHERE key_progect = ? and date = ?""",
            (key, date,))
        return self.cursor.fetchall()
    ...
    def change_type_of_menu_produce(self):
        ...
        result = self.cursor.execute(
            """UPDATE options_table SET menu_choose = 2 WHERE id = 1""") and self.connection.commit()
        return result

    def change_type_of_menu_montage(self):
        """Меняем меню на монтажное подразделение"""
        result = self.cursor.execute(
            """UPDATE options_table SET menu_choose = 3 WHERE id = 1""") and self.connection.commit()
        return result

    def change_type_of_menu_common(self):
        """Общее меню"""
        result = self.cursor.execute(
            """UPDATE options_table SET menu_choose = 1 WHERE id = 1""") and self.connection.commit()
        return result
